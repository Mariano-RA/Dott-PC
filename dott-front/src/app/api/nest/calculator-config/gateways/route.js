...
export async function POST(request) {
  ...
  try {
    ...
    const settings = await proxyPost(`${apiUrl}/calculator-settings/gateways`, body, { accessToken });
    return finalizeResponse(cookieJar, NextResponse.json({ settings }, { status: 200 }));
  } catch (error) {
    const status = error?.response?.status || 500;
    const message = getUpstreamErrorMessage(error, "Error al crear pasarela");
