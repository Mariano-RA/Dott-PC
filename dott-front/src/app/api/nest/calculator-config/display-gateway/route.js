...
export async function POST(request) {
  ...
  try {
    ...
    const settings = await proxyPost(`${apiUrl}/calculator-settings/display-gateway`, body, {
      accessToken,
    });
    return finalizeResponse(cookieJar, NextResponse.json({ settings }, { status: 200 }));
  } catch (error) {
    const status = error?.response?.status || 500;
    const message = getUpstreamErrorMessage(error, "Error al elegir pasarela de vitrina");
