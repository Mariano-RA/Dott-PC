...
export async function POST(request) {
  ...
  try {
    ...
    const data = await proxyPost(`${apiUrl}/categories/dictionary/bulk`, body, { accessToken });
    return finalizeResponse(cookieJar, NextResponse.json(data));
  } catch (error) {
    const status = error?.response?.status || 500;
    const message = getUpstreamErrorMessage(error, "Error al agregar mapeos en lote");
